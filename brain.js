...
function buildGraph(data) {
  ...
  const wingIndex = Object.fromEntries(wings.map((w, i) => [w, i]));

  crystals.forEach((crystal, index) => {
    const wing = crystal.wing || 'general';
    const normalized = Math.max(0, Math.min(1, ((crystal.importance_score || 0) + 0.5) / 1.6));
    const wingAngle = (wingIndex[wing] / Math.max(wings.length, 1)) * Math.PI * 2;
    const localAngle = (index / Math.max(crystals.length, 1)) * Math.PI * 2 * 1.9;
    const wingRadius = graphSpread * (0.55 + (wingIndex[wing] % 3) * 0.12);
    const orbitRadius = 28 + normalized * 48 + (index % 5) * 6;
    const x = Math.cos(wingAngle) * wingRadius + Math.cos(localAngle) * orbitRadius;
    const z = Math.sin(wingAngle) * wingRadius + Math.sin(localAngle) * orbitRadius;
    const y = ((wingIndex[wing] || 0) - (wings.length - 1) / 2) * 40 + (normalized - 0.5) * 150 + Math.sin(localAngle * 1.7) * 18;
    const radius = 3.2 + normalized * 5.4 + ((crystal.entities || []).length * 0.12);
    const color = crystal.actor === 'jupiter'
      ? 0xc4b5fd
      : crystal.actor === 'system'
        ? 0xf9a8d4
        : crystal.actor === 'assistant'
          ? 0x93c5fd
          : (SELF_STATE_COLORS[crystal.self_state] || 0x7dd3fc);
    const geometry = pickCrystalGeometry(crystal, radius, index);
    const material = new THREE.MeshStandardMaterial({
      color,
      emissive: color,
      emissiveIntensity: 0.45 + normalized * 0.85,
      roughness: 0.24,
      metalness: 0.18,
      transparent: true,
      opacity: 0.92,
    ...
    const timelineY = computeTimelineY(crystal, crystals);
    mesh.position.set(x + separationOffset(index, 0) * 6, timelineMode ? timelineY : y + separationOffset(index, 1) * 8, z + separationOffset(index, 2) * 6);
    mesh.userData = { id: 'crystal:' + crystal.id, kind: 'crystal', data: crystal, label: crystal.title || crystal.summary || ('Crystal #' + crystal.id) };
    graphGroup.add(mesh);
    nodeMap[mesh.userData.id] = { id: mesh.userData.id, rawId: crystal.id, kind: 'crystal', data: crystal, label: mesh.userData.label, mesh, baseX: x, baseY: y, baseZ: z, timelineY, wing, room: crystal.room || 'memories', actor: crystal.actor || 'user', radius, visualType: mesh.geometry.type };
    nodes.push(nodeMap[mesh.userData.id]);
  });

  entities.forEach((entity, index) => {
    const angle = (index / Math.max(entities.length, 1)) * Math.PI * 2;
    const radiusOrbit = graphSpread * 0.35 + (index % 7) * 10;
    const x = Math.cos(angle) * radiusOrbit;
    const z = Math.sin(angle) * radiusOrbit;
    const y = ((index % 9) - 4) * 16;
    const radius = 1.6 + Math.max(0.1, Math.min(1, entity.salience || 0.3)) * 3.2;
    const geometry = index % 2 === 0 ? new THREE.OctahedronGeometry(radius, 1) : new THREE.TetrahedronGeometry(radius * 1.08, 0);
    const material = new THREE.MeshStandardMaterial({
      color: 0x9dd6b4,
      emissive: 0x86efac,
    ...
    const mesh = new THREE.Mesh(geometry, material);
    mesh.position.set(x + separationOffset(index, 0) * 5, y + separationOffset(index, 1) * 6, z + separationOffset(index, 2) * 5);
  ...
  relations.forEach((relation) => {
    const source = nodeMap[relation.source_type + ':' + relation.source_id];
    const target = nodeMap[relation.target_type + ':' + relation.target_id];
    if (!source || !target) return;
    const color = RELATION_COLORS[relation.relation] || 0x8ca4ff;
    const material = new THREE.LineBasicMaterial({ color, transparent: true, opacity: 0.3 + Math.min(0.3, (relation.weight || 0.5) * 0.22) });
    const geometry = new THREE.BufferGeometry().setFromPoints([source.mesh.position.clone(), target.mesh.position.clone()]);
    const line = new THREE.Line(geometry, material);
    const pulseGeometry = new THREE.SphereGeometry(relation.relation === 'mentions' ? 0.65 : 0.9, 8, 8);
    const pulseMaterial = new THREE.MeshBasicMaterial({ color, transparent: true, opacity: 0.0 });
    const pulse = new THREE.Mesh(pulseGeometry, pulseMaterial);
    pulse.visible = false;
    line.userData = { key: source.id + '->' + target.id, relation: relation.relation };
    graphGroup.add(line);
    graphGroup.add(pulse);
    links.push({ key: line.userData.key, source: source.id, target: target.id, relation: relation.relation, weight: relation.weight || 0.5, line, pulse, phase: (Math.abs(relation.source_id + relation.target_id) % 100) / 100, animated: (relation.weight || 0.5) >= 0.5 || relation.relation === 'co_occurred' });
  });
...
function pickCrystalGeometry(crystal, radius, index) {
  ...
  if ((crystal.entities || []).length >= 4) return new THREE.DodecahedronGeometry(radius * 1.04, 0);
...
function renderInspector(node) {
  ...
  if (node.kind === 'crystal') {
    ...
    body.innerHTML = '' +
      ...
      '</div>' +
      (crystal.created_at ? '<div class="meta-label">Created</div><div>' + escapeHtml(new Date(crystal.created_at * 1000).toLocaleString()) + '</div>' : '') +
      '<div class="meta-label">Entities</div>' +
      '<div class="pill-row">' + ((crystal.entities || []).map(e => '<span class="pill">' + escapeHtml(e) + '</span>').join('') || '<span class="inspect-empty">None</span>') + '</div>';
  } else {
...
window.setCameraPreset = function(mode) {
  if (mode === 'ambient') fitCameraToGraph();
  else if (mode === 'focus') setOrbitDistance(22);
  else if (mode === 'timeline') { timelineMode = true; document.getElementById('timelineBtn').textContent = 'Timeline: ON'; loadGraph(); }
};
