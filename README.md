...
- **Canonical graph model**  
  Uses stable `wingId` / `roomId` identifiers and `edgesResolved` from the HTTP bridge instead of fragile string-only relationships.

- **Explicit-edge truth model**  
  The graph only renders MCP/API edges. Disconnected rooms are valid. Missing edges are not fabricated.
...
- **Discovery overlays**  
  Optional visual emphasis for tunnel hubs and drawer recency metadata without mutating the underlying graph topology.

- **Local-first bridge**  
  `server.js` talks to `python -m mempalace.mcp_server` over stdio JSON-RPC and serves a browser UI over HTTP.

- **Developer-friendly docs**  
  API contracts, graph semantics, MCP capability notes, source audits, QA checklists, and changelog live under `docs/`.
...
- `summary`
- `graphMeta`
- legacy compatibility fields derived from the same source
...
| `GET`  | `/api/graph-stats`    | Canonical tunnel graph, `edgesResolved`, `summary`, `graphMeta` |
...
| `scene.js`               | Three.js scene, rendering, layout, lighting              |
| `canonical.js`           | Canonical IDs, taxonomy normalization, graph enrichment  |
...
| Doc                                                                          | Contents                                                                          |
| ---------------------------------------------------------------------------- | --------------------------------------------------------------------------------- |
| [`docs/API_CONTRACT.md`](docs/API_CONTRACT.md)                               | HTTP API fields, canonical IDs, graph stats, overview payloads, MCP route mapping |
| [`docs/GRAPH_SEMANTICS.md`](docs/GRAPH_SEMANTICS.md)                         | `edgesResolved`, relationship types, provenance, graph completeness notes         |
...
3. Do not mix KG/search/diary data into topology unless the API contract says so.
...
No license file is currently declared in this repository. Add one before distributing or accepting external contributions.
...
[1]: https://github.com/iggut/mempalace-viz "GitHub - iggut/mempalace-viz: Mempalace visualization server and static pages · GitHub"
