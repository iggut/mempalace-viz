...
- **Explicit MCP/API edges** — Rows from MemPalace tools (`find_tunnels`, taxonomy), normalized to `edgesResolved` (stock: **tunnels**). Drawer reads via `/api/list-drawers` / `/api/drawer` are content-only, not graph edges.
